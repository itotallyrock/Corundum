...
pub const Square = enum(u6) {
    ...
    pub fn from_rank_and_file(r: Rank, f: File) Square {
        return @enumFromInt(@intFromEnum(r) * 8 + @intFromEnum(f));
    }

    pub fn to_bitboard(self: Square) BoardMask {
    ...
    fn is_shiftable(self: Square, comptime direction: BoardDirection) bool {
    ...
    pub fn shift(self: Square, comptime direction: BoardDirection) !Square {
        if (!self.is_shiftable(direction)) {
...
pub const Rank = enum(u3) {
    ...
    pub fn ep_rank_for(comptime player: Player) Rank {
        if (player == .White) {
            return ._3;
        } else {
            return ._6;
    ...
    pub fn promotion_rank_for(comptime player: Player) Rank {
        if (player == .White) {
            return ._8;
        } else {
            return ._1;
...
pub const File = enum(u3) {
    ...
    pub fn promotion_square_for(self: File, comptime player: Player) Square {
        return Square.from_rank_and_file(Rank.promotion_rank_for(player), self);
    }

    pub fn ep_square_for(self: File, comptime player: Player) Square {
        return Square.from_rank_and_file(Rank.ep_rank_for(player), self);
    }
};
...
pub const BoardMask = struct {
    ...
    pub const lines = struct {
        pub fn aligned_along(comptime from: Square, comptime to: Square, comptime alignment: SlidingPieceRayDirections) bool {
            if (from == to) return false;
            if (alignment == .cardinal) {
                return from.file() == to.file() or from.rank() == to.rank();
            } else {
                const rank_diff: u8 = @intFromFloat(@fabs(@as(f32, @floatFromInt(@as(i32, @intFromEnum(from.rank())) - @as(i32, @intFromEnum(to.rank()))))));
                const file_diff: u8 = @intFromFloat(@fabs(@as(f32, @floatFromInt(@as(i32, @intFromEnum(from.file())) - @as(i32, @intFromEnum(to.file()))))));
                return rank_diff == file_diff;
            }
        }
        pub fn aligned(comptime from: Square, comptime to: Square) bool {
            return aligned_along(from, to, .cardinal) or aligned_along(from, to, .diagonal);
        }
        pub fn are_aligned(comptime a: Square, comptime b: Square, comptime c: Square) bool {
            return !through(a, b).logicalAnd(c.to_bitboard()).isEmpty();
        }
        ...
        pub fn through(from: Square, to: Square) BoardMask {
            ...
            inline for (.{.cardinal, .diagonal}) |direction| {
                if (aligned_along(from, to, direction)) {
                    return from.to_bitboard()
                        .ray_attacks(BoardMask.EMPTY, direction)
                        .logicalAnd(to.to_bitboard().ray_attacks(BoardMask.EMPTY, direction))
                        .logicalOr(to.to_bitboard())
                        .logicalOr(from.to_bitboard());
                }
            }
        ...
        pub fn between(from: Square, to: Square) BoardMask {
            ...
            inline for (.{.cardinal, .diagonal}) |direction| {
                if (aligned_along(from, to, direction)) {
                    return from.to_bitboard()
                        .ray_attacks(to.to_bitboard(), direction)
                        .logicalAnd(to.to_bitboard().ray_attacks(from.to_bitboard(), direction));
                }
            }
    ...
    pub fn num_squares(self: BoardMask) u6 {
        return @popCount(self.mask);
    }

    pub fn pop_square(self: *BoardMask) ?Square {
        if (self.isEmpty()) {
            return null;
        }
    ...
    pub fn occluded_fill(self: BoardMask, occluded: BoardMask, comptime direction: BoardDirection) BoardMask {
        if (self.isEmpty()) {
            return EMPTY;
        }
    ...
    fn ray_attack(self: BoardMask, occupied: BoardMask, comptime direction: BoardDirection) BoardMask {
        return self.occluded_fill(occupied, direction).shift(direction);
    }

    fn ray_attacks(self: BoardMask, occupied: BoardMask, comptime direction: SlidingPieceRayDirections) BoardMask {
        if (direction == .cardinal) {
            return self.ray_attack(occupied, .north).logicalOr(self.ray_attack(occupied, .south)).logicalOr(self.ray_attack(occupied, .east)).logicalOr(self.ray_attack(occupied, .west));
        } else {
            return self.ray_attack(occupied, .northWest).logicalOr(self.ray_attack(occupied, .northEast)).logicalOr(self.ray_attack(occupied, .southEast)).logicalOr(self.ray_attack(occupied, .southWest));
        }
    }

    fn king_attacks(self: BoardMask) BoardMask {
    ...
    fn knight_attacks(self: BoardMask) BoardMask {
    ...
    pub fn attacks(self: BoardMask, occupied: BoardMask, comptime piece: NonPawnPiece) BoardMask {
        switch (piece) {
            .king => return self.king_attacks(),
            .knight => return self.knight_attacks(),
            .bishop => return self.ray_attacks(occupied, .diagonal),
            .rook => return self.ray_attacks(occupied, .cardinal),
            .queen => return self.ray_attacks(occupied, .cardinal).logicalOr(self.ray_attacks(occupied, .diagonal)),
        }
    }
};
...
pub const RelativeDirection = enum {
    ...
    pub fn to_direction(self: RelativeDirection, comptime perspective: Player) BoardDirection {
        return switch (self) {
...
fn testSquareToBoardMask(comptime square: Square, comptime expected: BoardMask) !void {
    ...
    try expectEqual(expected, square.to_bitboard());
}
...
fn testRayAttack(comptime direction: BoardDirection, sliders: BoardMask, occupied: BoardMask, comptime expected: BoardMask) !void {
    const expectEqualDeep = @import("std").testing.expectEqualDeep;
    try expectEqualDeep(expected, sliders.ray_attack(occupied, direction));
}
test "BoardMask ray attack works" {
    try testRayAttack(.northEast, Square.A1.to_bitboard(), BoardMask.EMPTY, BoardMask { .mask = 0x8040_2010_0804_0200 });
    try testRayAttack(.north, Square.A1.to_bitboard(), BoardMask { .mask = 0xffff }, BoardMask { .mask = 0x100 });
    try testRayAttack(.north, Square.H1.to_bitboard(), BoardMask { .mask = 0xffff }, BoardMask { .mask = 0x8000 });
    try testRayAttack(.north, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x1010_1010_0000_0000 });
    try testRayAttack(.south, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x0010_1010 });
    try testRayAttack(.east, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0xE000_0000 });
    try testRayAttack(.west, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x0F00_0000 });
    try testRayAttack(.northEast, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x0080_4020_0000_0000 });
    try testRayAttack(.southWest, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x80402 });
    try testRayAttack(.northWest, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x0102_0408_0000_0000 });
    try testRayAttack(.southEast, Square.E4.to_bitboard(), Square.E4.to_bitboard(), BoardMask { .mask = 0x0020_4080 });
}
...
fn testRayAttacks(comptime direction: SlidingPieceRayDirections, sliders: BoardMask, occupied: BoardMask, comptime expected: BoardMask) !void {
    const expectEqualDeep = @import("std").testing.expectEqualDeep;
    try expectEqualDeep(expected, sliders.ray_attacks(occupied, direction));
}
...
test "BoardMask Rook attacks works" {
    try testAttacks(.rook, Square.A1.to_bitboard(), BoardMask.EMPTY, .{ .mask = 0x0101_0101_0101_01FE });
    try testAttacks(.rook, Square.H1.to_bitboard(), BoardMask.EMPTY, .{ .mask = 0x8080_8080_8080_807F });
    try testAttacks(.rook, Square.A1.to_bitboard(), .{ .mask = 0xFFFF }, .{ .mask = 0x102 });
    try testAttacks(.rook, Square.H1.to_bitboard(), .{ .mask = 0xFFFF }, .{ .mask = 0x8040 });
    try testAttacks(.rook, Square.B4.to_bitboard(), .{ .mask = 0x2200_3300_0802 }, .{ .mask = 0x0202_1D02_0202 });
    try testAttacks(.rook, Square.D4.to_bitboard(), BoardMask.FULL, .{ .mask = 0x0008_1408_0000 });
    try testAttacks(.rook, Square.F6.to_bitboard(), .{ .mask = 0x2000_0000_0000 }, .{ .mask = 0x2020_DF20_2020_2020 });
    try testAttacks(.rook, Square.H1.to_bitboard(), .{ .mask = 0x80 }, .{ .mask = 0x8080_8080_8080_807F });
    try testAttacks(.rook, Square.C4.to_bitboard(), .{ .mask = 0x0004_2500_1000 }, .{ .mask = 0x0004_3B04_0404 });
    try testAttacks(.rook, Square.G6.to_bitboard(), .{ .mask = 0x4000_F800_0000 }, .{ .mask = 0x4040_BF40_4000_0000 });
}

test "BoardMask Bishop attacks works" {
    try testAttacks(.bishop, Square.A1.to_bitboard(), BoardMask.EMPTY, .{ .mask = 0x8040_2010_0804_0200 });
    try testAttacks(.bishop, Square.D4.to_bitboard(), BoardMask.FULL, .{ .mask = 0x0014_0014_0000 });
    try testAttacks(.bishop, Square.F6.to_bitboard(), .{ .mask = 0x2000_0000_0000 }, .{ .mask = 0x8850_0050_8804_0201 });
    try testAttacks(.bishop, Square.H1.to_bitboard(), .{ .mask = 0x80 }, .{ .mask = 0x0102_0408_1020_4000 });
    try testAttacks(.bishop, Square.C4.to_bitboard(), .{ .mask = 0x0020_0140_0402_4004 }, .{ .mask = 0x0020_110A_000A_1020 });
    try testAttacks(.bishop, Square.G6.to_bitboard(), .{ .mask = 0x4000_F800_0000 }, .{ .mask = 0x10A0_00A0_1000_0000 });
}

test "BoardMask Queen attacks works" {
    try testAttacks(.queen, Square.D4.to_bitboard(), BoardMask.FULL, .{ .mask = 0x001C_141C_0000 });
    try testAttacks(.queen, Square.D4.to_bitboard(), .{ .mask = 0x2000_0000_0000 }, .{ .mask = 0x0809_2A1C_F71C_2A49 });
    try testAttacks(.queen, Square.H1.to_bitboard(), .{ .mask = 0x80 }, .{ .mask = 0x8182_8488_90A0_C07F });
    try testAttacks(.queen, Square.F3.to_bitboard(), .{ .mask = 0x0038_0062_2000 }, .{ .mask = 0x00A8_705E_7088 });
    try testAttacks(.queen, Square.G6.to_bitboard(), .{ .mask = 0x4000_F800_0000 }, .{ .mask = 0x50E0_BFE0_5000_0000 });
}

test "BoardMask lines are_aligned works" {
    const expectEqual = @import("std").testing.expectEqual;
    try expectEqual(true, BoardMask.lines.are_aligned(.A2, .A4, .A6));
    try expectEqual(true, BoardMask.lines.are_aligned(.A2, .A4, .A8));
    try expectEqual(false, BoardMask.lines.are_aligned(.B2, .A4, .A8));
    try expectEqual(false, BoardMask.lines.are_aligned(.A2, .B4, .A8));
    try expectEqual(false, BoardMask.lines.are_aligned(.A2, .A4, .B8));
    try expectEqual(false, BoardMask.lines.are_aligned(.A2, .B4, .B8));
    try expectEqual(true, BoardMask.lines.are_aligned(.B2, .B4, .B8));
    try expectEqual(true, BoardMask.lines.are_aligned(.H1, .A1, .C1));
    try expectEqual(false, BoardMask.lines.are_aligned(.H1, .A1, .C2));
    try expectEqual(true, BoardMask.lines.are_aligned(.H8, .A1, .D4));
}
